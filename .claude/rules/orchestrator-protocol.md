...
7. 대상 경로는 `00-target-path.md` 에 기록한다. 환경변수로 export 하지 않는다 — 훅은 별도 프로세스로 실행되어 Bash 도구에서 export 한 임의 변수를 받지 못한다.
...
- **사용 빈도** (A7) — 생성될 하네스가 얼마나 자주 사용될지. header: `사용 빈도`. 옵션:
  1. `1회성·실험` — POC, 학습용, 단발 작업
  2. `저빈도 (주 1-2회 이하)` — 솔로 도구, 개인 워크플로우
  3. `중빈도 (주 3회 ~ 매일)` — 일상 도구, 본인 + 소수 사용자
  4. `고빈도·운영 (매일 다회 또는 팀 인프라)` — 다중 사용자, 운영 환경
  **용도** — Phase 3-6 에이전트 프롬프트의 `[Usage Intent]` 로 전달. Dim 14 (Scope-Intent Match) 의 입력. 트랙 판별 10번째 조건의 입력.
- **운영 성숙도** (A8) — 하네스가 동작해야 할 운영 강도. header: `운영 성숙도`. 옵션:
  1. `개인 도구` — 본인만 사용, 장애 = 본인 불편
  2. `팀 공유 도구` — 동료 N명 사용, 장애 영향 제한적
  3. `운영 인프라` — 다운타임 = 비즈니스 영향, 복구·감사 요구
  **용도** — A7과 함께 `[Usage Intent]` 로 전달. Session Recovery / Artifact Versioning / Pipeline Review Gate 자동 부여 여부의 판별 입력.
- **작업 복잡도 자기 평가** (A9) — 사용자가 추정한 본 작업 자체의 복잡도. header: `복잡도`. 옵션:
  1. `단순 (단일 기능, 명확한 입출력)`
  2. `중간 (다단계, 분기 있음)`
  3. `복잡 (외부 시스템·검토 게이트 다수, 다중 워크플로우)`
  **용도** — Phase 3-6 의 워크플로우 분기·에이전트 수 결정 기준. A10 옵션 본문 추정의 입력.
- **적정 규모 합의** (A10) — **암묵적 합의 방지 게이트**. AI 가 A1~A9 + 사용자 발화 + 미발화 맥락 (도메인·사용 강도·운영 성숙도·복잡도) 을 종합 추론하여 3가지 규모 시나리오를 옵션으로 제시. header: `적정 규모`. 옵션 (각 옵션 description 에 정량 견적: 에이전트 수, reviewer 수, playbook 수, 예상 파일 수, 예상 소요, 운영 비용 힌트):
  1. `슬림 (3-5 SKILL.md / 에이전트, reviewer 0, ~10 파일, 15-25분)` — 단순 솔로 도구, MRSO 같은 운영 코드 수준
  2. `중간 (5-8 에이전트, reviewer 1-2, ~25 파일, 35-50분)` — 검토 게이트 1단, 운영 가능 수준
  3. `풀 (10+ 에이전트, reviewer 3+, ~50 파일, 60분+)` — 운영 인프라급, HITL gate 다수, Session Recovery
  **AI 책임**: 옵션 본문 description 에 각 옵션의 추정 사유 (예: "회의록 + 솔로 + 주 1-2회 → 슬림 권장: 동일 도메인 운영 코드가 9 SKILL.md 로 운영 중") 와 트레이드오프를 명시한다. **모델 자체 판단의 `(권장)` 라벨 부착 금지** — `question-discipline.md` "Recommended Label Discipline" 준수. 추론 표현은 description 본문에만.
  **용도** — Phase 3-6 에이전트 프롬프트의 `[Agreed Scope]` 로 전달. Dim 14 의 핵심 기준선. Phase 5 직후 Scope Confirmation Gate 의 기준선. 트랙 판별 10번째 조건의 입력.
...
| Phase 0 초기 발화에 "--fast" / "빠르게" + 도메인 답변 "해당 없음" | 소환 안 함 | Fast Mode 의 도메인 미명시 케이스 — "빠르게" 단독으로는 Phase 2.5 를 스킵하지 않음. 사용자가 도메인을 명시했으면 Fast Mode 라도 소환 (도메인 리서치는 인터뷰 부담이 아닌 자동 리서치) |
...
```
Agent(
  subagent_type: "harness-architect:phase-domain-research",
...
## Fast Keyword Semantics

"빠르게" / "--fast" / "Fast Track" 키워드는 **트랙 단축이 아니라 운영 모드 전환** 신호다. 잘못된 해석이 v0.10.x 까지의 incident 의 핵심 원인이었으므로 본 문서가 단일 권위 정의를 가진다.
...
- 매 Phase 후 산출물 규모가 A10 합의·A7/A8 신호와 매칭되는지 Advisor (Dim 14) 가 자체 견제.

**X (의미가 아닌 것)**:
- Fast Track 트랙 진입 (Phase 1-2 만 수행 + 최소 산출물 + 이후 Phase 스킵) — 본 문서에서 "Fast Track" 용어는 deprecated.
- 산출물 규모 강제 축소 — A10 합의가 `풀` 이면 풀 규모로 진행.
- Advisor 생략. Model Confirmation Gate 생략. Scope Confirmation Gate 생략. `[ASK]` 무시.
- Phase별 분산 질문을 그냥 없앤 것 — 질문 *총량* 은 풀 모드와 비슷, *시점 분포* 만 초반 집중.
...
오케스트레이터는 다음 사유를 thinking 안에서 인용하여 Advisor/Gate 를 생략할 수 없다. 위반 시 Phase 9 final-validation "Advisor Completion Audit" 가 누락된 `advisor_status` 산출물을 [BLOCK] 으로 감지:
- "사용자가 빠르게 원함" → Advisor 생략 정당화 (금지)
- "사용자가 빠르게 원함" → Model Confirmation Gate 생략 정당화 (금지)
- "사용자가 빠르게 원함" → Scope Confirmation Gate 생략 정당화 (금지)
- "사용자가 빠르게 원함" → `[ASK]` 무시 정당화 (금지)
- "BLOCK 없으니 상식적으로 생략" → Skip Gate 외 경로 생략 정당화 (금지)

Skip Gate 의 명시 조건 (Phase ∈ {3-4, 7-8} AND Escalations `[BLOCKING]`/`[ASK]` 0건 AND 구조 검증 1회차 통과 AND 누적 `manual_override` 없음 AND `review_exempt: false`) 만 인정. 본 절의 사유는 견제 비용을 줄이는 의미가 아니라 **AI 의 자율 판단 책임을 강화하는 신호** 다.

### "빠르게" + "에이전트 파이프라인" 동시 발화 처리
...
아래 10개 조건을 모두 AND로 확인한다. **모든 조건이 충족될 때만 경량 트랙**이다.
...
| **사용 강도·합의 규모 (A7 + A8 + A10)** | `## Context for Next Phase` → Pre-collected Answers (A7/A8/A10) | **A7 ∈ {1회성·실험, 저빈도 (주 1-2회 이하)} AND A8 = `개인 도구` AND A10 ∈ {슬림, 중간}**. 이 조합 충족 시 강한 경량 신호 — 위 8개 조건 중 1-2개 불충족이어도 경량 트랙 제안 유지 (사용자 최종 선택). A10 = `풀` 이면 본 조건 불충족 → 풀 트랙 |
...
| **3-4** | **phase-design** | **workflow-design → pipeline-design (순차)** | **Phase 3+4 통합. 단일 에이전트가 두 플레이북을 순차 Read·실행하여 `02-workflow-design.md` + `03-pipeline-design.md` 두 산출물 분리 생성. Phase 2.5 산출물 있으면 Read. Advisor 1회만 실행.** |
...
```
Agent(
  subagent_type: "harness-architect:security-auditor",
...
1. Phase N은 `{3-4, 7-8}` 중 하나 (Phase 3-4 통합 산출물 1세트 단위로 평가; Phase 5·6은 설계 품질이 중요하므로 skip 금지)
2. Phase N 에이전트 산출물의 `## Escalations` 섹션이 "없음" 또는 `[NOTE]` 항목만 존재 (`[BLOCKING]`/`[ASK]` 0건). Phase 3-4 통합 산출물의 경우 **02 와 03 의 Escalations 를 합산**하여 평가.
3. 산출물 구조 검증(`validate-phase-artifact.sh`)이 1회차에 exit 0 통과. **Phase 3-4 의 경우 `02-workflow-design.md` AND `03-pipeline-design.md` 두 파일 모두 1회차 exit 0 이어야 한다** — 한 파일만 통과한 상태에서 Skip Gate 진입 금지.
...
**Skip Gate 외 경로 생략 금지 (자체 우회 차단)**: 위 5개 조건을 모두 충족하지 않은 상태에서 오케스트레이터가 thinking 내부 사유로 Advisor 호출을 생략하는 것은 금지된다. 다음 사유는 인용 불가:
- "사용자가 빠르게 원함" / "사용자가 Fast Mode 라" — Fast Keyword Semantics 섹션 참조. "빠르게" 는 Advisor 생략 정당화가 아님.
- "BLOCK 없으니 상식적으로 생략" — Skip Gate 조건 1·2·3 의 명시 평가를 우회하는 추측은 금지.
- "Phase 9 final-validation 이 잡아줄 것" — Phase별 Advisor 의 책임을 final-validation 으로 미루는 것은 금지.

위반 시 결과: 해당 Phase 산출물의 `advisor_status` frontmatter 가 누락되거나 `pass` 외 값으로 기록되지 않으며, Phase 9 final-validation 의 "Advisor Completion Audit" 단계가 이를 [BLOCK] 으로 감지한다.
...
```
Agent(
  subagent_type: "harness-architect:red-team-advisor",
...
## Scope Confirmation Gate (Phase 5 직후)

### 목적
Dim 14 (Scope-Intent Match) 이 BLOCK 을 놓치거나 사용자가 Dim 14 `[ASK]` 에서 "그대로 진행" 선택한 경우의 **최종 안전망**. Phase 5 (`phase-team`) 완료 직후, 사용자가 처음으로 *전체 에이전트 인벤토리·reviewer 수·HITL gate 수*를 한눈에 볼 수 있는 시점에 한 번 더 확인한다. v0.10.x 까지의 incident 에서 50+ 파일 규모를 사용자가 Phase 9 완료 후에야 인지한 구조적 문제를 해소.

### 실행 조건
- Phase 5 Advisor 리뷰가 `pass` / `note` / `manual_override` 로 종료
- 다음 중 하나 이상 충족 (복잡도 게이트 우회 불가 — 단순 프로젝트도 적용):
  - 에이전트 수 ≥ 5
  - reviewer 수 ≥ 2 (Pipeline Review Gate 자동 부여 포함)
  - HITL gate ≥ 1
- A10 합의 ≠ `풀` 인데 위 조건 충족 시 **발화 강도 강화** — AskUserQuestion 옵션에 "A10 합의와 현재 규모 격차" 정량 명시

### 동작

1. 오케스트레이터가 `docs/{요청명}/04-agent-team.md` Agent Model Table + Pipeline Review Gate 분류 + HITL gate 수를 한 표로 정리하여 사용자에게 노출:
   ```
   | 카테고리 | 현재 | A10 합의 |
   |---|---|---|
   | 에이전트 수 | 11 | 슬림 (3-5) |
   ...
   | 예상 파일 수 | 50+ | 슬림 (~10) |
   ```
2. A10 합의가 있으면 격차 명시 (예: "A10 = 슬림 (3-5 에이전트), 현재 = 11 에이전트. 격차: +6~8")
3. AskUserQuestion (header `규모 확정`, 3선택):
   - `이 규모로 진행` — 사유 명시 의무. "(권장)" 라벨 자동 부착 금지 (question-discipline.md "Recommended Label Discipline" 준수)
   - `축소 — Phase 4-5 재소환` — 사용자가 줄일 항목 지정 (reviewer 제거 / sub-agent 통합 / HITL gate 1개 제거 등) → `phase-design` 또는 `phase-team` 재소환 프롬프트에 축소 요청 전달
   - `의도 재확인` — Phase 0 A10 답변 보정 후 Phase 3 부터 재실행 (대규모 재작업 — 사용자 명시 승인 시에만)
4. 확정 시 `04-agent-team.md` frontmatter 에 `scope_confirmation: confirmed` 또는 `scope_confirmation: manual_override` 기록. "축소" 선택 시 재소환 결과 산출물에 `scope_confirmation: reduced` 기록.
5. 재소환 상한: 2회. 소진 시 "현재 규모 수용 / 수동 편집" 2선택 (Advisor 루프 패턴과 동일)

### Model Confirmation Gate 와의 관계
별개 게이트. **실행 순서**: Phase 5 Advisor → **Scope Confirmation Gate** → Phase 6 (`phase-skills`) → Phase 6 Advisor → **Model Confirmation Gate** → Phase 7-8.
- Scope Gate: *규모* 확인 (얼마나 큰가)
- Model Confirmation Gate: *모델 배정* 확인 (얼마나 비싼가)
- 둘 다 통과해야 Phase 7-8 진입

### 재개 시 안전 체크
재개 시 `04-agent-team.md` frontmatter 의 `scope_confirmation` 필드가 `confirmed`/`manual_override`/`reduced` 가 아니면 Gate 재진입 대상. 누락 시 (구형 산출물) 풀 트랙으로 재진입.

### thinking 내 인용 금지
"사용자가 빠르게 원함" → Scope Gate 생략 정당화 (금지). Fast Keyword Semantics 섹션 참조.
...
| **Phase 3-4** | docs/{name}/01-discovery-answers.md (Phase 2.5 실행 시 02b-domain-research.md 도 선택적 입력) |
| Phase 5 | docs/{name}/02-workflow-design.md AND docs/{name}/03-pipeline-design.md (Phase 3-4 통합 에이전트가 두 파일 모두 생성한 후 Phase 5 진입) |
...
2. **구조 검증** (Bash 직접 호출):
   ...
   ```
   - exit 0 → Step 3(미해결 [ASK] 차단)으로 진행
   - exit 1 → stderr의 누락 항목을 에이전트에게 전달하며 "다음 항목을 Edit으로 보완 후 재저장" 지시 → 1회만 재검증
     - 재검증 exit 0 → Step 3 진행
     - 재검증 exit 1 → `AskUserQuestion` "수동 편집 / Phase 스킵" (2선택)
3. **미해결 [ASK] / [BLOCKING] 차단** (자체 결정 우회 방지):
   `validate-phase-artifact.sh` 의 stdout 에 포함된 Escalation 카운트 (`ASK=N, BLOCKING=M` 형식) 를 파싱한다. **N + M ≥ 1 이면 다음 Step (Advisor) 진입 차단**:
   - 모든 미해결 항목을 한 묶음으로 `AskUserQuestion` 으로 사용자에게 일괄 노출 (최대 4개씩, 5개 이상이면 분할)
   - 사용자 답변을 산출물 `## Escalations` 섹션에 `→ [RESOLVED] {사용자 답변}` 으로 Edit 추가하고, 다음 Phase 에이전트 프롬프트의 `[Confirmed User Decisions]` 누적에도 포함
   - 모든 항목 해소 후 Step 4 (Advisor) 진입
   - 이는 v0.10.x 까지 오케스트레이터가 `[ASK]` 항목을 thinking 내에서 자체 결정 처리하여 사용자에게 노출하지 않은 사례 (incident §8 #6 — Phase 3-4 "통합 8 vs 분리 14" [ASK] 사용자 비공지) 의 구조적 차단 레이어
4. Advisor 실행 (기존 Red-team Advisor 프로토콜 유지)

**포맷 실패와 Advisor BLOCK은 완전히 분리된 루프**다. 포맷 재검증이 Advisor 루프에 진입하지 않으며, Advisor BLOCK이 포맷 재검증을 유발하지 않는다. **[ASK] 차단 루프도 별개** — Advisor BLOCK 이 [ASK] 차단을 유발하지 않으며, [ASK] 차단이 Advisor 루프에 진입하지 않는다.
...
| 3 (02-workflow-design.md) | 워크플로우 스텝 목록 (이름, 목적, 사용자 트리거 여부), 스텝 간 의존성, 완료 조건. **Phase 3-4 통합 에이전트 `phase-design` 이 작성하며, 작성 직후 동일 컨텍스트에서 Phase 4 단계로 진입한다.** |
| 4 (03-pipeline-design.md) | 에이전트 목록 (이름, 역할, 모델, 쓰기 범위), 에이전트별 스킬 매핑, 실행 순서/패턴, 소통 포인트, **메인 세션 역할 (라우터 only / 직접 실행 가능)**. **Phase 3-4 통합 에이전트가 작성하며, 이 산출물 완료 시점이 통합 에이전트의 종료 지점**. |
...
**기록 가치 게이트 (noise 억제)**: 검토한 발상을 전부 적지 않는다. 다음 세 조건을 **모두** 충족하는 대안만 기록한다:
1. **실재한 후보** — 짚어보고 진지하게 비교한 선택지다. strawman·즉시 탈락한 발상은 제외.
2. **실 트레이드오프 기반 기각** — "그냥 더 나쁘다"가 아니라 *무엇을 얻고 무엇을 잃어서* 기각했는지 한 줄로 말할 수 있다.
3. **맥락 없이는 재제안될 만함** — 이 기각 사유를 모르는 후속 Phase가 같은 대안을 합리적으로 다시 꺼낼 수 있다.

세 조건을 모두 채우지 못하는 대안은 적지 않는다 — 기각 목록이 길어지면 후속 Phase가 진짜 제약을 가려내지 못한다. 채택안이 처음부터 자명했다면 "검토된 대안 없음"으로 둔다.
...
Phase 7-8 완료 시에는 MCP 처리 결과를 **항상** 1줄 함께 노출한다 (MCP 단계의 사용자 가시성 보장 — 후보 0건 경로에서도 사용자가 MCP가 평가됐음을 인지). Phase 8 은 "리스트 + 설정 스니펫" 모델이므로 등록 수가 아닌 **추천 수** 를 노출한다:
"✅ Phase 7-8 완료. MCP: {추천 N건 — 사용자 설치 필요 / 추천 후보 없음 — Phase 8 스킵}. Advisor 리뷰 중..."

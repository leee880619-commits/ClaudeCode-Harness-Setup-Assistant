...
나머지 질문(Q5~Q9)은 서브에이전트가 스캔 결과를 기반으로 Escalations에 기록하고,
...
2. 승인 시: Phase 3(workflow) → Phase 4(pipeline) → Phase 5(team)를 순차 실행하되,
   각 Phase 에이전트에 "에이전트 프로젝트" 맥락을 전달하여 설계 방향을 사전 안내
...
| 3 | phase-workflow | workflow-design | |
| 4 | phase-pipeline | pipeline-design | |
| 5 | phase-team | agent-team | |
| 6 | phase-skills | skill-forge | 복수 에이전트 SKILL 생성 시 TeamCreate 고려 |
...
- Phase 1-2, 7-8, 9: Advisor 경량 실행 (NOTE만 수집, BLOCK/ASK 없으면 자동 통과)
...
| 시작 Phase | 필수 산출물 |
|-----------|-----------|
| Phase 3 | docs/{name}/01-discovery-answers.md |
...
|-------|-------------------------------|
| 1-2 | 프로젝트 유형, 기술 스택, 솔로/팀, 에이전트 프로젝트 여부, 디렉터리 구조 요약, 기존 설정 존재 여부 |

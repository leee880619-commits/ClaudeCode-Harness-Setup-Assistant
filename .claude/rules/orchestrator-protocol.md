...
아래 8개 조건을 모두 AND로 확인한다. **모든 조건이 충족될 때만 경량 트랙**이다.
...
| 에이전트 신호 (3-A) | `## Escalations` → [ASK] 에이전트 프로젝트 감지 항목 | 없음 (NOTE도 없음 — 보수적 기본값; 스펙 대비 강화된 조건) |
...
2. AskUserQuestion으로 사용자에게 트랙 결정을 고지:
   - 경량 트랙: "스캔 결과 8개 경량 트랙 기준을 모두 충족합니다 (솔로, 비에이전트, 코드베이스 규모·배포·서비스 복잡도 신호 없음). 경량 트랙을 제안합니다 (약 25~35분, 8~10회 LLM 호출). 풀 트랙(60분+, 18회+ 호출)보다 빠르게 MVP 하네스를 완성합니다."
...
`track: lightweight`로 분류된 프로젝트 (8개 판별 기준 모두 충족 — 솔로·비에이전트·코드베이스·배포·서비스 복잡도 신호 없음):

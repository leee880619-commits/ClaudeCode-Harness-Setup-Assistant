...
1. **Phase 1-2 (fresh-setup)**: 프로젝트 스캔 시 관련 파일 감지
   - `docs/architecture/code-map.md` 또는 유사 파일 존재 → `code-navigation.md` 채택 제안
2. **Phase 3 (workflow-design)**: 워크플로우 설계 시 공용 규칙 채택 여부 Escalation
3. **Phase 7-8 (hooks-mcp-setup)**: 공용 훅이 필요한 경우

각 단계에서 Escalation으로 사용자에게 채택 여부를 확인한다.

...
# tmux: display-message (소켓 통신, Claude Code 우회) → status bar에 5초 표시
# 일반 터미널: /dev/tty 직접 출력 (isatty=False 문제 우회)
...
[[ "$UPDATE_AVAILABLE" != "yes" ]] && exit 0

# stdout — Claude 컨텍스트용 (SessionStart hook additional context)
echo "harness-architect v${LATEST} 출시됨 (현재 설치: v${CURRENT}). 사용자에게 \`/plugin update harness-architect\` 실행을 안내할 수 있음."

# tmux: display-message 는 tmux 서버 소켓으로 직접 통신 — Claude Code 가 가로챌 수 없음
if [[ -n "${TMUX:-}" ]] && command -v tmux &>/dev/null; then
    tmux display-message -d 5000 "⬆ harness-architect v${LATEST} 출시 | /plugin update harness-architect"
fi

# 일반 터미널: /dev/tty 직접 출력으로 stdout/stderr 우회 시도
# (훅 환경에서 stderr.isatty()=False 이므로 Claude Code 가 가로채는 문제 우회)
{ printf '\n\033[1m\033[36m⬆  harness-architect\033[0m \033[90mv%s\033[0m → \033[1m\033[32mv%s\033[0m  |  업데이트: \033[33m/plugin update harness-architect\033[0m\n\n' "$CURRENT" "$LATEST" > /dev/tty; } 2>/dev/null || true

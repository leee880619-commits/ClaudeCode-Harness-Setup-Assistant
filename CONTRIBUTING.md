...
- [ ] 예시 디렉터리당 파일 수 3개 이하 유지 (초과 시 유지보수 부담 증가)
...
- 예: `phase-setup.md` → `name: phase-setup` → 플러그인 설치 컨텍스트의 `subagent_type: "harness-architect:phase-setup"`.
- **네임스페이스 prefix 필수**: 이 레포가 플러그인 매니페스트(`plugin.json`)에 `"name": "harness-architect"` 로 등록되어 있으므로, **모든 `subagent_type` 호출은 `harness-architect:` prefix를 포함**해야 합니다. prefix를 빠뜨리면 설치된 플러그인 런타임에서 `Agent type not found` 에러가 발생합니다. (개발 시 `claude --plugin-dir .` 도 동일하게 네임스페이스가 적용됩니다.)

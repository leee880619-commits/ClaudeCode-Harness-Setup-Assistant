...
- **예외 — 상반 신호 동시 발행 (v0.11.2 추가)**: 다음 페어는 *동일 파일 경로를 참조하지만 의미가 정반대* 이므로 **병합 금지**. 양쪽 모두 통합 보고서에 별도 항목으로 노출하고 SSoT 충돌 카운트에서도 제외한다:
  - **ops-audit Dim A (Session Recovery 부재 → RISK-HIGH)** ↔ **fit-audit Dim 3.5 F7 (Session Recovery 자동 부여 + A8 ≠ 운영 인프라 → MAJOR-DRIFT)**: 전자는 "복구 프로토콜 없음" 을 지적하고 후자는 "복구 프로토콜 과잉" 을 지적. 동일 파일(`02-workflow-design.md` / 03-pipeline-design.md`) 참조 시 Jaccard 알고리즘이 오병합 위험. 양쪽 모두 노출하여 사용자가 "정말 운영 인프라가 필요한가" 자체를 결정하도록 유도.
  - 일반화: **`(ops-audit Dim X 권고 = 추가/강화)` ↔ `(fit-audit Dim Y 권고 = 축소/제거)` 페어는 의미적 모순이므로 모두 별도 노출**. 자동 감지 휴리스틱: ops-audit 항목 본문에 "추가/필요/부재/누락" 어휘 + fit-audit 항목 본문에 "과잉/축소/제거/Drift" 어휘가 모두 등장하면 모순 페어 후보 → 병합 skip.

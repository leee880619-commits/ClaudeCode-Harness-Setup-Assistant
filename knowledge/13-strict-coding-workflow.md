...
Phase 1-2에서 다음 신호를 수집한다 (에이전트 프로젝트가 아닌 전제):

- **2개 이상** 해당 → `[ASK]`로 기록하여 의사결정 요청 (preset 채택 제안)
- **정확히 1개** 해당 → `[NOTE]`로 기록하여 정보 전달만 (사용자가 Phase 3에서 원하면 채택 가능)
- **0개** → 기록하지 않음
- **예외** — 단일 신호가 **#7(사용자 의지 발화)** 인 경우 `[ASK]`로 승격. 사용자가 명시적 품질 의지를 표현한 상황에서 질문이 떠지 않는 것을 방지.
...
1. Phase 1-2 fresh-setup에서 신호 감지:
   - 2개 이상 → Escalations에 `[ASK] Strict Coding 6-Step 권장`
   - 정확히 1개 → Escalations에 `[NOTE] Strict Coding 6-Step 소개` (Phase 3에서 사용자가 원하면 채택 가능)
   - **예외**: 단일 신호가 신호 #7(사용자 의지 발화)이면 `[ASK]`로 승격
2. 오케스트레이터 처리:
   - `[ASK]` → AskUserQuestion으로 확인 (적용 / 커스텀 설계 / 단순 워크플로우)
   - `[NOTE]` → 텍스트 보고만 수행. 사용자가 Phase 3 직전에 명시적으로 요청하면 채택 플로우 진입

...
| **L6: 인프라** | `.cursor/mcp.json`, `.cursor/hooks.json` | `.claude/settings.json` (hooks 섹션) + 프로젝트 루트 `.mcp.json` (MCP) | hooks 설정은 `settings.json`으로 통합. **MCP 서버 설정은 `settings.json`이 아니라 프로젝트 루트 `.mcp.json`(project 스코프)으로 변환** — Claude Code는 `settings.json`에서 MCP를 읽지 않는다 |
...
│
├── .mcp.json                                          ← .cursor/mcp.json 변환 (project 스코프)
│                                                         MCP 서버 설정. VCS 커밋 대상. settings.json 아님
│
├── .claude/
│   ├── settings.json                                  ← .cursor/hooks.json 변환
│   │                                                     hooks 설정, permissions (MCP 설정 제외)
│   │

...
3. **Target Project Guardrail** — 대상 프로젝트 작업 중에는 플러그인 자체 파일을 수정하지 않습니다. Claude Code의 `permissions.deny` 로 강제합니다 (v1.0.5 이전의 `ownership-guard.sh` 훅은 무동작이어서 제거 — 8.2 참조).
...
│   ├── hooks/
│   │   ├── hooks.json           ← plugin hook declarations (PostToolUse only)
...
├── commands/                    ← 7 files on disk, 3 exposed in autocomplete (whitelist via plugin.json `commands` array, v0.9.2~)
│   ├── harness-setup.md         ← 노출: /harness-architect:harness-setup (신규/재개 구축)
│   ├── audit.md                 ← 노출: /harness-architect:audit (기존 하네스 통합 감사, v0.9.0~)
│   ├── help.md                  ← 노출: 정적 사용법 안내
│   ├── harness-audit.md         ← 숨김(v0.9.2): harness-auditor 에이전트 자연어 소환용 규약
│   ├── ops-audit.md             ← 숨김(v0.9.2): ops-auditor 에이전트 자연어 소환용 규약
│   ├── fit-audit.md             ← 숨김(v0.9.2): fit-auditor 에이전트 자연어 소환용 규약
│   └── update.md                ← 숨김(v0.9.2): /plugin update harness-architect 내장 명령 안내용 내부 문서
...
- `hooks.json`: `bash ${CLAUDE_PLUGIN_ROOT}/.claude/hooks/syntax-check.sh`
...
| 3-4 | 워크플로우 설계 + 파이프라인 설계 (통합 단일 에이전트, 산출물 2개 분리 유지) | phase-design | workflow-design → pipeline-design (순차) |
| 5 | 에이전트 팀 편성 (Teams/Agent/SendMessage) | phase-team | agent-team |
| 6 | SKILL/playbook 작성 | phase-skills | skill-forge |
| 7-8 | 훅 설치 + MCP 추천(리스트·스니펫) | phase-hooks | hooks-mcp-setup |
...
| 사후 감사 개별 (v0.9.2~) | 구성 정합성 단독 — 에이전트 자연어 소환 경로 (슬래시커맨드는 whitelist 정책으로 autocomplete 에서 제외) | harness-auditor | harness-audit |
| 사후 감사 개별 (v0.9.2~) | 런타임 부채 단독 — 에이전트 자연어 소환 경로 (슬래시커맨드는 whitelist 정책으로 autocomplete 에서 제외) | ops-auditor | ops-audit |
| 사후 감사 개별 (v0.9.2~) | 프로젝트 적합성·드리프트 단독 — 에이전트 자연어 소환 경로 (슬래시커맨드는 whitelist 정책으로 autocomplete 에서 제외) | fit-auditor | fit-audit |
...
- 위반 시도는 `final-validation` Step 3 일관성 검증에서 잡힙니다.
...
- [Dim 8] phase-setup과 phase-design이 둘 다 CLAUDE.md 본문 수정 → 단일 소유자 원칙 위배
...
이 플러그인은 훅을 **하나만** 등록합니다. 훅은 매 Write/Edit마다 OS 프로세스를 새로 띄우므로, 값을 못 하는 훅은 비용만 남깁니다.

### 8.1 syntax-check.sh (PostToolUse: Write|Edit)

생성물의 기본 유효성을 즉시 검증합니다.

- 입력: **stdin JSON** 의 `tool_input.file_path` (Windows의 `C:\\...` 이스케이프를 정규화)
- 범위: `.claude/` 하위 파일, `CLAUDE.md`, `docs/{요청명}/NN-*.md` Phase 산출물. **그 외 파일은 즉시 `exit 0`** (인터프리터를 부르지 않음)
- JSON 파일: `jq` 우선, 없으면 `python3`/`python`. 셋 다 없으면 검증 생략
- `settings.json`: `"Bash(*)"` 같은 위험 허용 패턴 + `deny` 누락 탐지
- YAML frontmatter 닫힘 검증 (`---` 짝 매칭)
- 위반 시 **`exit 2`** — stderr가 Claude에게 전달되어 후속 수정을 유도합니다. (`exit 1` 은 Claude Code에서 차단이 아닙니다.)
- `hooks.json` 에 `"timeout": 10`, `"shell": "bash"` 를 명시합니다.

계약은 `scripts/test-hooks.sh` 가 고정합니다.

### 8.2 ownership-guard.sh (v1.0.5 에서 제거)

`PreToolUse(Write|Edit)` 쓰기 범위 가드가 있었으나 **한 번도 동작한 적이 없어 제거**했습니다. 세 가지가 겹쳐 있었습니다.

1. 존재하지 않는 `$CLAUDE_TOOL_INPUT` 환경변수를 읽어, 항상 첫 분기에서 `exit 0` 했습니다. (실제 입력은 stdin JSON)
2. 설령 입력이 들어와도 Windows 절대경로(`C:\...`)가 `/` 로 시작하지 않아 상대경로로 오인되고, 프로젝트 루트를 앞에 붙인 뒤 "허용" 판정으로 fail-open 했습니다.
3. 차단에 `exit 1` 을 썼습니다. Claude Code에서 차단은 `exit 2` 이므로, 로직이 살아났더라도 아무것도 막지 못했습니다.

또한 가드가 참조하던 `$TARGET_PROJECT_ROOT` 는 **훅에 전달되지 않습니다.** 훅은 별도 프로세스로 실행되며, Bash 도구에서 `export` 한 변수는 그 셸과 함께 사라집니다. 대상 프로젝트 범위를 알 방법이 애초에 없었습니다.

무동작 훅이 파일 하나당 프로세스 1개를 띄우고, `timeout` 미지정으로 기본 600초를 살아남아 실제로 수백 개가 누적된 사례가 있습니다.

**쓰기 범위 보호는 Claude Code의 `permissions.deny`** 로 합니다 (`.claude/settings.json`). 훅 기반 가드를 되살리려면 대상 경로를 **파일로** 전달하는 경로부터 설계해야 합니다.

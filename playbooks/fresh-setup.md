...
- A6. **품질 축** (멀티 선택) — `프론트엔드 디자인·UX` / `Strict Coding` / `보안·컴플라이언스` / `에이전트 파이프라인` / `해당 없음`. Step 3-B/3-D/3-E 자동 판별의 공식 입력. 트랙 판별 9번째 조건의 입력.
- A7. **사용 빈도** — `1회성·실험` / `저빈도 (주 1-2회 이하)` / `중빈도 (주 3회 ~ 매일)` / `고빈도·운영`. Dim 13 (Scope-Intent Match) 의 입력. 트랙 판별 10번째 조건의 입력. 누락 시 Escalations에 `[BLOCKING] A7 답변 누락` 기록.
- A8. **운영 성숙도** — `개인 도구` / `팀 공유 도구` / `운영 인프라`. Session Recovery / Artifact Versioning / Pipeline Review Gate 자동 부여 판별 입력. 누락 시 `[BLOCKING] A8 답변 누락`.
- A9. **작업 복잡도 자기 평가** — `단순` / `중간` / `복잡`. Phase 3-6 의 워크플로우 분기·에이전트 수 결정 기준. 누락 시 `[NOTE] A9 미응답 → 중간 기본값`.
- A10. **적정 규모 합의** — `슬림` / `중간` / `풀` 중 택1. AI 가 A1~A9 + 발화 + 미발화 맥락을 종합 추론해 옵션 description 으로 추정을 노출한 뒤 사용자가 선택한 값. **Phase 3-6 의 척추 입력** — 본 스킬 산출물의 `## Context for Next Phase` 에 `Agreed Scope: {답변}, AI Recommended: {AI 추정}, Justification: {추론 사유}` 형식으로 기록. 누락 시 `[BLOCKING] A10 답변 누락 — Phase 0 압축 인터뷰 실패. 오케스트레이터 재진입 필요`.

프롬프트에 A1~A10 (A4 옵션 제외 9개 필수 항목) 의 답변이 **누락** 되거나, **출처가 AskUserQuestion#N 또는 $ARGUMENTS prefill (경로 한정) 이 아닌** 항목이 있으면 Escalations에 `[BLOCKING] A{N} 답변 누락 또는 출처 불명 (현재 출처='{값}')` 으로 기록하여 오케스트레이터가 재수집하도록 요청한다. 스킬이 직접 사용자에게 묻지 않는다.

각 답변에는 **출처 필드가 동반** 되어야 한다 — 오케스트레이터가 프롬프트에 다음 형식으로 전달:

```
[Pre-collected Answers]
- A1 (이름·설명): "{값}" — source: AskUserQuestion#1
- A2 (유형): "{값}" — source: AskUserQuestion#1
- A3 (솔로/팀): "{값}" — source: AskUserQuestion#1
- A5 (성능 수준): "{값}" — source: AskUserQuestion#2
- A6 (품질 축): "{값(멀티)}" — source: AskUserQuestion#2
- A7 (사용 빈도): "{값}" — source: AskUserQuestion#2
- A8 (운영 성숙도): "{값}" — source: AskUserQuestion#3
- A9 (작업 복잡도): "{값}" — source: AskUserQuestion#3
- A10 (적정 규모): "{값}" — source: AskUserQuestion#3
```

출처가 `발화 추출` / `AI 추정` / `자동 결정` / `기본값 적용` / 누락 인 항목은 **BLOCKING 발행 대상**. Phase 0 silent inference (3슬롯만 발화하고 A1·A2·A6·A9 를 발화 추출로 채움) 의 사후 검증 레이어. 자세한 정의: `.claude/rules/question-discipline.md` "Free-form Utterance Inference Discipline". 배경 incident: CHANGELOG.md [Unreleased].

### 출처 토큰 외부 검증 (Patch D-1)

본 스킬 실행 시 첫 동작으로 `00-target-path.md` 의 `## Pre-collected Answers` 섹션을 Read 한 뒤, 다음을 자동 검증한다:

1. **9개 필수 행 존재** — A1·A2·A3·A5·A6·A7·A8·A9·A10 행이 모두 표에 있는지
2. **출처 토큰 유효성** — 각 행의 "출처 호출#" 열이 `AskUserQuestion#1` / `AskUserQuestion#2` / `AskUserQuestion#3` / `$ARGUMENTS prefill` (경로 한정) 중 하나인지
3. **옵션 라벨 원문 매칭** — A2·A3·A5·A6·A7·A8·A9·A10 행의 "옵션 라벨 원문" 열이 `commands/harness-setup.md` "Phase 0 인터뷰 옵션 description 기준" 섹션의 라벨 카탈로그와 정확히 일치하는지 (자유 텍스트 응답 항목 A1·경로는 N/A 허용)

오케스트레이터가 출처 토큰을 자기 신고로 위조 (예: 실제로는 발화 추출인데 "AskUserQuestion#1" 이라고 표에 적은 경우) 한 경우, 옵션 라벨 원문이 카탈로그와 불일치할 가능성이 높다. 라벨 매칭 실패 시 `[BLOCKING] A{N} 옵션 라벨 원문 카탈로그 불일치 — 출처 위조 의심` Escalation 으로 오케스트레이터 재발화 요청.

추가로, 오케스트레이터가 `00-target-path.md` Write 직후 `scripts/validate-phase-artifact.sh` 를 실행하지 않고 phase-setup 을 소환한 경우에도, 본 스킬은 첫 동작으로 동일 검증을 수행하여 다층 방어를 형성한다.

**A5/A6/A7/A8/A9/A10 값은 산출물(`01-discovery-answers.md`) "Pre-collected Answers" 에 원값(예: `균형형`, `슬림`) 그대로 기록한다.** 라벨 전문("경제형 - Haiku 위주...")을 CLAUDE.md·rules·agents 등 대상 프로젝트 생성물에 복제하지 않는다(메타 누수 회피).

## Fast Mode

프롬프트에 `--fast` 또는 "빠르게 해줘"가 포함된 경우의 **운영 모드** (트랙 단축이 아님). 의미 정본은 `.claude/rules/orchestrator-protocol.md` "Fast Keyword Semantics" 섹션. 본 스킬은 그 정의에 따라 다음 동작을 수행:

1. **압축 깊이 인터뷰는 오케스트레이터의 책임**: 본 스킬 실행 시점에는 이미 Phase 0 에서 A1~A10 의 9개 필수 항목 (A4 옵션 제외) 이 모두 수집되어 프롬프트로 전달된 상태여야 한다. A1·A2·A3·A5·A6·A7·A8·A9·A10 중 하나라도 누락이거나, **출처가 AskUserQuestion#N 이 아닌** 항목 (발화 추출/AI 추정/자동 결정/기본값) 이 있으면 `[BLOCKING] A{N} 답변 누락 또는 출처 불명` Escalation 으로 오케스트레이터가 재수집하도록 요청. 본 스킬은 합리적 기본값으로 채워 *진행하지 않는다* — "Q5~Q9 합리적 기본값" 패턴 및 "발화 추출로 옵션 발화 생략" 패턴 모두 폐기 (배경: CHANGELOG.md).

   **Step 0 — 검증 영수증 재실행 의무 (다층 방어, R-2)**: 본 스킬의 첫 동작은 다음 2단계다:
   - (a) `${TARGET_PROJECT_ROOT}/docs/{요청명}/00-target-path.md` Read
   - (b) `bash ${CLAUDE_PLUGIN_ROOT}/scripts/validate-phase-artifact.sh ${TARGET_PROJECT_ROOT}/docs/{요청명}/00-target-path.md` Bash 직접 실행
   exit 1 인 경우 본 스킬은 즉시 `[BLOCKING] Phase 0 검증 실패: {스크립트 stderr 인용}` Escalation 발행하고 더 이상 진행하지 않는다. Bash 호출 자체가 실패 (스크립트 없음 / bash 미가용) 인 경우 `[BLOCKING] Phase 0 검증 영수증 실행 불가: {에러 메시지}` Escalation. 이 Step 0 는 오케스트레이터가 Phase 0 검증 영수증 (`.claude/rules/orchestrator-protocol.md` 동명 섹션) 을 실제로 실행했는지 *재확인*하는 독립 레이어다 — 오케스트레이터의 자기 신고 ("검증 통과") 만 신뢰하지 않고 본 스킬이 직접 재실행하여 silent inference 의 마지막 우회 경로를 차단한다.
2. 스캔 결과 자동 수집 (아래 Step 1)
3. 사전 수집 답변(A1~A10) + 스캔 결과 + A10 합의로 기본 하네스 생성:
   - CLAUDE.md (스캔 기반)
   - settings.json (기본 deny + 감지된 빌드 명령 allow)
   - .gitignore 업데이트
   - Intent Gate 베이스라인 (Step 3-F)
4. **A10 = `슬림` 이면 추가 동작**: Step 3-D (Strict Coding 자동 판별) 와 Step 3-E (보안·컴플라이언스) 자동 주입을 [ASK] 로 강등 — 사용자가 명시 요청한 경우에만 주입. A10 합의 자체가 슬림 의지 표명이므로 자동 부여는 적정선 초과.
5. Phase 1-2 완료. Next Steps 에는 풀 모드와 동일한 다음 Phase 안내를 기록 (Fast Mode 가 Phase 진행을 단축하지 않으므로).
6. Phase 1-9 진행 중 추가 사용자 질문 최소화 — Phase 0 인터뷰로 충분해야 한다는 가정. 부득이한 `[ASK]` 는 Escalations 에 즉시 기록 (오케스트레이터의 Phase Gate `[ASK]` 차단 단계가 사용자에게 노출).

**Fast Mode 가 단축하는 것**:
- Phase별 분산 질문 횟수 (질문 시점이 초반에 집중되므로 후반 질문 감소)

**Fast Mode 가 단축하지 않는 것**:
- Phase 수 (1-2/2.5/3-4/5/6/7-8/9 모두 실행)
- Advisor 호출 (각 Phase 후 정상 호출)
- Scope Confirmation Gate / Model Confirmation Gate (정상 발화)
- 산출물 규모 (A10 합의가 풀이면 풀 규모)

Fast Mode 완료 목표 (Phase 1-2 본 스킬 한정): 슬림 A10 기준 15분 이내, 풀 A10 기준 25-30분.
...
- **사용 강도·운영 성숙도 발화** (A7/A8 답변과의 정합 검증용): "주 N회 사용", "매일 돌린다", "팀 공유", "운영용", "POC", "단발", "1회성", 특정 사용자 수 언급 등. 발화 신호와 A7/A8 답변이 어긋나면 (예: 발화에 "팀 매일 운영" 인데 A7=저빈도) Escalations 에 `[ASK] A7/A8 답변과 발화 신호 불일치 — 사용자 확인 필요` 기록
...
1. **CLAUDE.md** (≤200줄):
   ...
   - **Anti-duplication gate (다중 에이전트 하네스 한정)** — **시점 보정**: CLAUDE.md는 Phase 1-2(이 Step)에서 작성되고, 에이전트 수는 Phase 5, SKILL.md 수는 Phase 6에서 확정된다. 따라서 이 Step에서는 **휴리스틱 사전 신호**로 다중 에이전트 가능성을 추정한다. 다음 중 **하나라도** 충족하면 게이트 발동:
     1. Phase 0 사전 답변 A6(품질 축)에 멀티 선택(2개 이상) 또는 `에이전트 파이프라인` 포함
     2. Step 3-A 에이전트 신호 점수가 [ASK] 임계 이상 (즉 Escalations에 `[ASK] 에이전트 프로젝트 감지` 기록 예정)
     3. Step 3-B Strict Coding 신호가 [ASK] (다중 역할 분리 요구)
     4. 사용자 발화에 "여러 에이전트", "팀", "오케스트레이터", "워크플로우 N개" 같은 멀티-에이전트 어휘
     예측이 빗나가도 안전: 게이트 발동 시 CLAUDE.md는 라우터 형태로 작성되며, Phase 5에서 에이전트가 1개로 확정되면 Phase 6 완료 후 phase-setup 재소환으로 워크플로우 본문을 보충 가능 (단일 소유자 원칙 유지). 게이트 미발동 시 CLAUDE.md는 기존 방식 그대로 작성되며 Phase 9 항목 14가 사후 감지.

     게이트 발동 시 CLAUDE.md에 워크플로우/파이프라인 상세(정체성·전략표·N-step·품질 기준·anti-pattern 정의)를 **본문으로 기재 금지**. 대신 다음을 둔다:
     - 워크플로우 매트릭스 표 1개 (이름·목적·진입 조건·SSoT SKILL.md 경로 — 행당 1줄)
     - 워크플로우별 핵심 원칙 한 줄 요약 + SSoT 위치 포인터 (예: "PORT anti-pattern은 `validate-port` SKILL.md에서 정의·검사")
     의도: CLAUDE.md는 라우터(router), SKILL.md는 매뉴얼(manual). 200줄 캡은 hard limit이며, 라우터 역할에 충실하면 보통 100줄 이내로 수렴. 단일 에이전트(에이전트 ≤ 1)이면 이 게이트는 권고만 (soft warning) — fragmentation 위험.
     SSoT — 양방향 교차 참조: 이 게이트의 임계값과 의미는 `.claude/rules/output-quality.md` Item 2 와 동일하다. 변경 시 양쪽 동시 수정.
   ...
   - **오케스트레이터 라우팅 프로토콜 섹션 (Step 3-D 에서 코드 프로젝트로 감지된 경우만 삽입, 비코드는 생략)**:
     ...
     - 해당 작업이 `mandatory_review` 로 분류된 파이프라인에 속함
     - 생성 / 결정 / 설계 / 계획 / 리서치 성격의 작업 (신규 기능, 새 문서, 새 스키마, 후보 선택, 전략 분기)
     - 산출물이 외부에 공개됨 (공개 API 문서, 릴리즈 노트, 배포 아티팩트)

     위에 해당하지 않는 **기존 코드의 국소 수정** (버그 픽스, 오타, 설정값 조정, 타입 보정) 은 커밋되더라도 S 등급으로 처리할 수 있다. 표의 "파일 3개 이하 변경" 이 그 뜻이다.
     S 등급이라도 리뷰어 스텝을 가진 파이프라인을 통과하는 중이라면 그 리뷰어 스텝은 생략하지 않는다.
...
- 기본 경로: "Phase 3-4: phase-design 에이전트 소환 권장 (워크플로우+파이프라인 통합)"

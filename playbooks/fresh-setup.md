...
프롬프트에 A1~A3, A7, A8, A10 답변이 **누락**된 항목이 있으면 Escalations에 `[BLOCKING] A{N} 답변 누락`으로 기록하여
...
1. **압축 깊이 인터뷰는 오케스트레이터의 책임**: 본 스킬 실행 시점에는 이미 Phase 0 에서 A1~A10 가 모두 수집되어 프롬프트로 전달된 상태여야 한다. A1~A3 / A7 / A8 / A10 중 하나라도 누락이면 `[BLOCKING] A{N} 답변 누락` Escalation 으로 오케스트레이터가 재수집하도록 요청. 본 스킬은 합리적 기본값으로 채워 *진행하지 않는다* — v0.10.x 의 "Q5~Q9 합리적 기본값" 패턴은 폐기.

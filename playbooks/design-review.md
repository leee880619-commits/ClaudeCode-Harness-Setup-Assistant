...
### Dimension 14: Scope-Intent Match (의도-규모 정합성, Phase 3·4·5·6 산출물에 항상 적용)

> **배경**: v0.10.x 까지 솔로 + "빠르게" + 주 1-2회 사용 시나리오에 11 에이전트 + 14 playbook + 5 reviewer + HITL 2-gate 같은 운영 인프라급 하네스가 생성됐다. 9-Phase 가 모두 정의대로 동작하고 Phase 9 final validation 도 통과했음에도 사용자가 "오버엔지니어링"으로 평가한 사례. 기존 Dim 1~13 은 "산출물이 목적에 기여하는가" 는 보지만 *"과한가"* 는 보지 않는다. 본 Dimension 이 그 빈틈을 메운다.

**검사 대상**: Phase 3-4 (`02-workflow-design.md`, `03-pipeline-design.md`), Phase 5 (`04-agent-team.md`), Phase 6 (`05-skill-specs.md`) 산출물.

**기준선 입력**: `docs/{요청명}/01-discovery-answers.md` 의 Pre-collected Answers 에 기록된 **A10 적정 규모 합의** (슬림/중간/풀), A7 사용 빈도, A8 운영 성숙도, A3 솔로/팀 여부, "빠르게" 키워드 감지 여부.

**비교 매트릭스 수집**: 산출물에서 다음 카운트:
- 에이전트 수 (Phase 4·5 산출물 기준)
- reviewer 수 (Pipeline Review Gate 자동 부여 + 명시 reviewer 합산)
- HITL gate 수
- playbook 수 (Phase 6 산출물 기준)
- `mandatory_review` 분류 수
- 자동 부여된 운영 인프라 패턴 (Session Recovery / Artifact Versioning / Complexity Gate S/M/L)

**임계치 룰** (모두 `[BLOCK]`, 단 산출물에 사용자 명시 요청 흔적이 있는 경우 `[ASK]` 강등):

| 룰 # | 조건 | 판정 |
|------|------|------|
| R1 | A10 = `슬림` AND 에이전트 수 ≥ 5 | [BLOCK] |
| R2 | A10 = `중간` AND 에이전트 수 ≥ 8 | [BLOCK] |
| R3 | A3 = 솔로 AND 에이전트 수 ≥ 7 | [BLOCK] |
| R4 | "빠르게" 키워드 감지 AND A10 답변 누락/공백 | [BLOCK] — Phase 0 압축 인터뷰 실패 의미. orchestrator 가 A10 재수집해야 함 |
| R5 | reviewer 수 ≥ 3 AND A8 ∈ {개인 도구, 팀 공유 도구} | [BLOCK] |
| R6 | HITL gate ≥ 2 AND A7 ∈ {1회성·실험, 저빈도 (주 1-2회 이하)} | [BLOCK] |
| R7 | playbook 수 ≥ 10 AND 에이전트당 평균 playbook ≥ 1.5 | [ASK] "playbook 단순화 검토 — 일부를 인라인 SKILL.md 로 통합 가능?" |
| R8 | Session Recovery / Artifact Versioning 자동 부여 AND A8 ≠ 운영 인프라 | [BLOCK] |
| R9 | A10 = `슬림` 인데 산출물에 Complexity Gate S/M/L 도입 | [BLOCK] |

**참조 비교 (있으면 사용)**: 사용자가 Phase 0 발화에서 동일 도메인 운영 코드를 명시 인용한 경우 (`01-discovery-answers.md` 의 `User-Declared Structure` 또는 본문에 URL/repo 경로 기록), 그 코드의 인벤토리 (대략 SKILL.md 수, 에이전트 수) 와 본 산출물 규모를 비교. 격차 **3배 이상** 이면 `[BLOCK] 인용된 운영 코드 ({이름}) 는 {N}개 SKILL.md / 0 에이전트로 운영. 본 산출물은 {M}개 에이전트 / {K}개 reviewer — 사유 불충분`.

**인용 의무**: BLOCK 발행 시 본문에 두 줄 명시:
1. 어느 사용자 발화·신호와 충돌하는지 (예: "A10 = 슬림, A7 = 저빈도, A8 = 개인 도구")
2. 어느 산출물 항목을 줄이면 해소되는지 (예: "reviewer 3종 → 0종 또는 통합 1종, sub-agent 8 → 3개")

**ASK 강등 경로**: 산출물의 `## Context for Next Phase` 또는 본문에 사용자가 명시적으로 요청한 흔적 (예: "사용자 명시: 'reviewer 필수 부여'") 이 인용되어 있으면 `[BLOCK]` → `[ASK]` 강등. 단 산출물 작성 에이전트가 자체 결정으로 적은 인용은 강등 사유가 아님 — `[Confirmed User Decisions]` 입력에 해당 항목이 있는지 교차 확인.

**복잡도 게이트 / Skip Gate 우회 금지**: 본 Dimension 은 Dim 6 (보안) 와 동일하게 **복잡도 게이트와 Advisor Skip Gate 모두 우회 불가**. 경량 트랙·Skip Gate 경량 Advisor 호출 시에도 Dim 14 는 항상 전체 실행. 이유: 산출물 규모 미스매치는 보안 위반과 같은 격의 사용자 피해 (시간·토큰·인지 부담 = 시스템 신뢰 침식).

**처리 경로** (Advisor 가 BLOCK 발행한 경우 오케스트레이터 동작):
1. 오케스트레이터는 AskUserQuestion 으로 3-선택 제시:
   - `A. 산출물 축소` — Phase N 에이전트 재소환, Dim 14 인용 사유 + 줄일 항목 제안을 프롬프트로 전달
   - `B. 의도 신호 보정` — 사용자가 A10 답변 정정 (슬림 → 중간 등). 정정 후 산출물 재평가 (Advisor 재실행)
   - `C. 그대로 진행` — 사유 텍스트 명시 후 산출물 frontmatter 에 `intent_scope_override: yes, reason: {사용자 사유}` 기록
2. 재소환 상한 2회. 소진 시 "현재 수용 / 수동 편집" 2선택

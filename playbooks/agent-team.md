...
각 에이전트에 대해 Escalations에 기록하여 확인:

- **모델**: opus (복잡한 설계) / sonnet (구현) / haiku (단순 검증)
...
- CLAUDE.md에 `## 에이전트 팀 구조` 섹션 추가
- 에이전트별 역할/모델/범위 명시
- 에이전트-스킬 소유권 매핑 테이블 (1 에이전트 : N 스킬, 각 스킬의 예상 저장 위치 포함) 포함
- 소통 패턴 다이어그램 포함
- 소유권 맵 포함 (해당 시)
...
- 모델 선택 시 비용/성능 트레이드오프를 Escalations에 포함.

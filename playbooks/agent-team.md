...
- **프리셋 주입 파일 소유권 보호**: Phase 1-2 산출물(`01-discovery-answers.md`)의 `## Context for Next Phase` 에 `프론트엔드 프리셋 주입 여부: yes` 가 기록돼 있으면, 대상 프로젝트 `.claude/agents/frontend-designer.md` · `.claude/agents/frontend-ux-reviewer.md` 두 파일은 **이미 주입 완료** 된 상태이므로 이 Phase 에서 재작성·덮어쓰기·`allowed_dirs` 변경을 금지한다. Agent Model Table 에는 해당 에이전트를 포함하되 비고에 "프리셋 주입 — 재작성 제외" 기록. 다른 에이전트를 새로 추가하는 것은 허용된다.
...
> **`model` 필드는 별칭만 사용 — 풀 ID 금지.** 값은 반드시 `opus` / `sonnet` / `haiku` 별칭 중 하나로 기록한다. `claude-opus-4-7`·`claude-sonnet-4-6` 같은 풀 모델 ID 를 쓰지 않는다. 별칭 `opus`/`sonnet`/`haiku`는 Claude Code가 **해당 패밀리의 최신 모델로 해석**한다 (공식 — Claude Code Model Configuration 문서: "`opus` — Uses the latest Opus model"). 실제 해석 결과는 하네스 파일이 아니라 *사용자의 실행 환경*에서 결정된다 — 프로바이더(Anthropic API·Claude Platform on AWS는 최신, Bedrock/Vertex/Foundry는 한 세대 뒤)와 `ANTHROPIC_DEFAULT_{OPUS,SONNET,HAIKU}_MODEL` env 오버라이드. 따라서 별칭을 박으면 사용자 환경 기준 항상 유효한 현행 모델로 실행되며, 버전 번호를 기억할 필요가 없다. 반대로 풀 모델 ID(`claude-sonnet-4-6` 등)는 **고정 스냅샷**이라 영구히 그 버전에 묶이고 끝내 retired되며, 생성 시 학습 데이터의 낡은 버전(`claude-sonnet-4-5` 등)으로 환각될 위험도 있다. Step 3 매트릭스의 `sonnet`/`opus`/`haiku` 표기를 그대로 frontmatter 에 옮긴다.
...
- 생성하는 `.claude/agents/*.md` 의 `model` 필드는 별칭(`opus`/`sonnet`/`haiku`)만 사용 — 풀 모델 ID 금지 (Step 5 참조). Phase 9 `final-validation` 이 별칭 위반을 BLOCK 으로 검출한다.

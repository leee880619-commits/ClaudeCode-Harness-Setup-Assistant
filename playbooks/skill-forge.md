...
### Description 작성 규율 (필수)

frontmatter의 `description`은 Claude Code가 스킬을 자동 매칭할 때 보는 **유일한 신호**다 (본문은 스킬 호출 시점에야 로딩). 잘못 쓰면 호출되어야 할 때 호출되지 않거나, 엉뚱한 때 호출된다. 다음 규율을 따른다:

- **3인칭 동작 서술** — "도와줍니다"가 아니라 "~을 생성한다 / ~을 검증한다"처럼 이 스킬이 하는 일을 동작으로 명시.
- **구조: what + 발동 조건** — 첫 문장은 *무엇을 하는가*. 이어서 *언제 발동하는가*를 구체적으로 명시한다. 발동 조건에는 사용자 발화 키워드·따옴표 호출구·파일 타입·상황을 빠짐없이 담는다 (예: `... 사용자가 "배포"를 언급하거나 staging 환경을 말하거나 .github/workflows/ 를 수정할 때 사용한다`).
- **간결하게** — 한 문단을 넘기지 않는다. 트리거는 빠짐없이 담되 산문을 늘이지 않는다.
- **time-sensitive 정보 금지** — 버전 번호·날짜·"새로운"/"최신" 같은 표현을 description에 넣지 않는다. 시간이 지나면 틀린 정보가 된다.
- 대상 프로젝트 고유 용어로 쓰되, 이 어시스턴트·Claude Code 메타 개념을 노출하지 않는다 (`output-quality.md` 메타 누수 금지와 동일).
...
### Self-Containment 원칙 (필수)
각 SKILL.md는 **자기완결적**이어야 한다. CLAUDE.md를 읽지 않아도 해당 스킬이 무엇을 어떻게
실행하는지 단독으로 이해 가능해야 한다. 워크플로우 정체성·전략표·N-step 흐름·품질 기준·
anti-pattern 정의는 SKILL.md 본문에 두며, CLAUDE.md에 동일 내용을 복제하지 않는다.
이는 `.claude/rules/output-quality.md` Item 2(SKILL.md = SSoT for workflow detail) 의
실현 메커니즘이다 — SKILL.md가 자기완결적이어야 CLAUDE.md가 라우터 역할로 축소될 수 있다.

**경로 무관 적용**: 이 원칙은 케이스 A(`.claude/skills/{name}/SKILL.md`, 사용자 진입점) 와
케이스 B(`playbooks/{name}.md`, 에이전트 전용 방법론) 모두에 동일하게 적용된다.
저장 위치는 메인 세션 노출 여부를 결정하지만, 자기완결성 요구는 위치와 무관하다.

...
MCP 서버: Q9에서 명시된 경우만 **추천 후보**로 기록한다 (등록·설치 실행은 사용자가 직접 수행 — 풀 트랙 `hooks-mcp-setup.md` Part 2 의 "리스트 + 설정 스니펫" 모델과 동일 책임 경계). 없으면 `MCP: 없음`으로 명시(미기록이 아닌 "없음 선언" — 풀 트랙 `## MCP Servers` 0건 표기와 동일 문구).
...
- 훅:
  - {훅 이름}: {목적} — {스크립트 힌트}
- MCP 추천: 없음 / {서버명, 목적} (사용자가 `claude mcp add --scope <scope>` 로 직접 등록)
...
- **운영 가드 필드 고정**: YAML frontmatter의 `session_recovery: not_applicable` / `artifact_versioning: idempotent` 는 경량 트랙 공식 선언으로 수정 금지. 풀 트랙 업그레이드 시 `phase-design` 에이전트가 재판정하여 본격 `## Session Recovery Protocol` 섹션을 신설한다.

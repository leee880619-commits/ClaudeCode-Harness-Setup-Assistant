...
오케스트레이터 프롬프트의 `[Agreed Scope]` 필드 값(`슬림` / `중간` / `풀`)을 산출물 설계에 반영한다. 이 값은 Phase 0 A10 적정 규모 합의 답변이며, 사용자가 명시 동의한 산출물 규모의 기준선이다.

**규모별 reviewer / HITL gate 부여 기준**:

| Agreed Scope | reviewer 수 상한 | HITL gate 수 상한 | mandatory_review 면제 가능 |
|--------------|------------------|--------------------|----------------------------|
| `슬림` | 0 (또는 사용자 명시 요청 시 1) | 0 (또는 1) | 가능 — `review_exempt: agreed_scope_slim` 명시 후 면제 |
| `중간` | 1-2 | 0-1 | 부분 가능 — 생성/결정 파이프라인은 면제 불가, 단순 변환은 면제 가능 |
| `풀` | 3+ 가능 | 1+ | 불가 — `pipeline-review-gate.md` 기본 규칙 적용 |

**면제 시 산출물 표기**: `mandatory_review` 분류라도 Agreed Scope = 슬림이면 다음 형식으로 면제 가능:
```
- pipeline: generate-meeting-log
  classification: mandatory_review
  review_exempt: agreed_scope_slim
  exempt_reason: "A10 = 슬림 합의 (01-discovery-answers.md). 솔로 도구·저빈도 사용에서 reviewer 자동 부여는 과적."
```

**Dim 14 (Scope-Intent Match) 와의 연계**: 본 입력 무시 시 Phase 4 Advisor 의 Dim 14 가 BLOCK 발화 (룰 R5: reviewer 수 ≥ 3 + A8 ∈ {개인 도구, 팀 공유 도구}, R9: A10 = 슬림인데 Complexity Gate S/M/L 도입 등). 본 섹션의 상한을 준수하면 Dim 14 자동 통과.
...
**적용 조건**: Phase 3(workflow-design) Step 4-B 가 `## Context for Next Phase` 에 "Complexity Gate Pipeline Contracts 생성 필요" 를 전달한 경우에만 이 스텝을 수행한다. 조건은 다음 OR 중 하나:
- 멀티 에이전트 / 에이전트 파이프라인 / 오케스트레이터 패턴
- strict-coding-6step 채택
- 코드베이스 규모 LoC ≥ 5,000 또는 파일 ≥ 100개

위 조건에 해당하지 않는 소규모 프로젝트는 **이 스텝 전체 스킵**. 오케스트레이터가 CLAUDE.md 의 "오케스트레이터 라우팅 프로토콜" 섹션(workflow-design Step 4-B 가 삽입)만으로 취사선택한다. 계약 테이블 없이 오케스트레이터 재량에 맡긴다.

적용 조건 해당 시, Phase 4는 각 등급별로 **에이전트 호출 체인과 산출물 계약**을 명시한다. 계약이 없으면 라우팅 프로토콜이 선언뿐이고 실제 경로 분기가 동작하지 않는다. 이 계약은 라우팅 프로토콜의 **하위 레이어 명세**이며 프로토콜을 대체하지 않는다.

**S 등급 계약** (메인 세션 직접 구현):
- 에이전트 소환: 0회
- 전제: ownership-guard 는 stdin JSON 에 `agent_type` 키가 없다는 사실로 메인 세션을 식별해 통과시킨다 (hooks-mcp-setup Step 2의 신원 판정 블록과 짝). **환경변수 플래그나 토큰 락 파일을 두지 않는다** -- 훅에 도달하지 않거나 자가 승인이라 인증이 성립하지 않는다.

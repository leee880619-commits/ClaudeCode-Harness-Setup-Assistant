...
v0.9.2 부터 슬래시 autocomplete 에는 위 2개 + `/harness-architect:help` 총 **3개만** 노출됩니다. 개별 auditor(구성·런타임·적합성) 는 `/harness-architect:audit` 내부에서 자동 병렬 호출됩니다. 특정 축만 실행이 필요하면 해당 auditor 에이전트(`harness-auditor` / `ops-auditor` / `fit-auditor`) 를 자연어로 직접 소환하세요.
...
**개별 축 실행** (고급): v0.9.2 부터 단일 축 슬래시커맨드는 autocomplete 에서 제거되었고, `/harness-architect:audit` 으로 통합되었습니다. 단일 축만 실행하려면 해당 auditor 에이전트를 자연어로 직접 소환:

- "harness-auditor 에이전트를 대상 경로 {경로} 로 실행해" — 구성 정합성만 (CRITICAL/HIGH/MEDIUM/LOW)
- "ops-auditor 에이전트를 대상 경로 {경로} 로 실행해"     — 런타임 부채만 (RISK-HIGH/MED/LOW)
- "fit-auditor 에이전트를 대상 경로 {경로} 로 실행해"     — 프로젝트 적합성만 (MAJOR-DRIFT-CRITICAL/MED · MINOR-DRIFT · ALIGN)

설계 철학·구성·Escalation 프로토콜의 상세는 [ARCHITECTURE.md](./ARCHITECTURE.md)를 보세요.

## 가드레일

- **대상 프로젝트 작업 중**: 이 플러그인 자체 파일을 수정하지 않음 (Claude Code `permissions.deny` 로 차단)
